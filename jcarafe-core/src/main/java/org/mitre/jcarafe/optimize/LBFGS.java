...
public class LBFGS {
  ...
  public static void lbfgs ( int n , int m , double[] x , double f , double[] g , boolean diagco , double[] diag , int[] iprint , double eps , double xtol , int[] iflag, boolean restart ) throws ExceptionWithIflag
	{
		boolean execute_entire_while_loop = false;

		if ( w == null || restart || w.length != n*(2*m+1)+2*m )
		{
		...
		while ( true )
		{
			...
			{
				iflag[0]=-1;
				System.out.println("info: " + info[0] + " iflag: " + iflag[0]);
