...
object InstanceSerializations extends DefaultProtocol {
  ...
  implicit def obsSourceMap : Format[ObsSource[String]] = {
    asProduct5({(lb: Int, obs: String, bg: Boolean, info: Option[Map[String,String]], condTable: Map[Int,Double]) => 
      new ObsSource[String](lb, obs, bg, info, condTable)}){(os: ObsSource[String]) =>        
        (os.label,os.obs.toString,os.beg,os.info,os.getCondProbTable)}
  }
