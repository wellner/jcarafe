...
object ConvertIOBToLex {
  ...
  val commaReg = "COMMA".r

  def mapObs(obs: String) = obs match {
    case "``" => "\""
    case "''" => "\""
    case a => commaReg.replaceAllIn(a,",")
  }
  ...
  def processNPChunkingData(lines: Iterator[String], os: java.io.OutputStreamWriter) = {
    var inside: Option[String] = None
    val lineFn = getLineInfoStd(""" \d+[ ]+\d+[ ]+(\S+)[ ]+(\S+)[ ]+(\S+).*""".r) _
    lines foreach { line =>
  ...
  def main(args: Array[String]) = {
    ...
    val tbMode = ((args.length > 2) && (args(2) equals "tb"))
    val os = new java.io.OutputStreamWriter(new java.io.BufferedOutputStream(ostr), "UTF-8")
    var lines = src.getLines()
    var inside: Option[String] = None
    val lineFn =
      if (std) getLineInfoStd(""" \d+[ ]+\d+[ ]+(\S+)[ ]+(\S+)[ ]+(\S+).*""".r) _
      else getLineInfoMayo("""(\S+) (\S+) (\S+)""".r) _
    lines foreach { line =>
      if (!((line.length < 1) || (line(0) == '#'))) {
        val (lab, pos, obs) = lineFn(line)
        if (lab == "O") {
